...
def get_schema(m, schema_list, output_dir, start_after=None, devicetype="UT"):
    failed_download = []
    getting_yet = True
    if start_after:
        getting_yet = False
    for s in schema_list:
        if devicetype == "CUCC":
            if s[0:4] != "ietf" and s[0:11] !="chinaunicom":
                continue
        if devicetype == "UT":
            if s[0:11] =="chinaunicom":
                continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--devicetype', type=str,
    ...
    if not args.skip_download:
        failed = get_schema(mgr, schema_list, args.output_dir, args.start_after, args.devicetype)
        for f in failed:
